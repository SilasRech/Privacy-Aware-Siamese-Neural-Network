...
if cnn_list.iloc[0]['classifier'] == 'gender':
    ...
    test_data = np.load('C:\\Users\\jonny\\Desktop\\log\\InputData\\test_data.json.npy')
    test_labels = np.load('C:\\Users\\jonny\\Desktop\\log\\InputData\\test_label.json.npy')
...
scores, accuracy = neural_network(x_eval=eval_data, y_eval=eval_labels, x_train=train_data, y_train=train_labels, loss=1,
                        x_test=test_data, y_test=test_labels, y_retest=retest_labels, y_retrain=retrain_labels ,x_retrain=retrain_data, x_retest=retest_data, utterance=True)
